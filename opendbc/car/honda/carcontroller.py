...
class CarController(CarControllerBase):
  ...
  def update(self, CC, CS, now_nanos):
    ...
    if CC.longActive:
      self.stop_accel = -0.2 if ((actuators.longControlState == LongCtrlState.stopping) and (actuators.accel >= -0.2)) else actuators.accel
      accel = self.stop_accel
      gas, brake = compute_gas_brake(self.stop_accel + hill_brake, CS.out.vEgo, self.CP.carFingerprint)
    else:
      accel = 0.0
      gas, brake = 0.0, 0.0
    ...
    if not CC.longActive:
      ...
      self.gas_error = self.stop_accel - CS.out.aEgo
      if (not CS.out.gasPressed) and (actuators.longControlState == LongCtrlState.pid):
        if self.gas_error != 0.0 and gas > 0.0:
          self.gasfactor = np.clip(self.gasfactor + self.gas_error / 1000 * (gas * 4.8), 0.1, 6.0)
        if self.gas_error != 0.0 and (gas - brake) > 0.0:
          self.speedfactor = np.clip(self.speedfactor + self.gas_error / 60 * (gas + wind_brake - brake) * 4.8, 0.1, 7.0)
          self.speed_addon = np.clip(self.speed_addon + self.gas_error / 120 * (gas + wind_brake - brake) * 4.8, 0.0, 99.0)

        # windfactor
        if self.gas_error != 0.0 and (not CS.out.brakePressed) and (not CS.out.gasPressed) and (CS.out.vEgo > 0.0):
          wind_adjust = 1 + (wind_brake * 4.8) / 1000
          self.windfactor = np.clip(self.windfactor * (wind_adjust if (self.gas_error > 0) else 1.0/wind_adjust), 0.1, 1.5)
      ...
      if self.frame % 2 == 0:
        ...
        if self.CP.carFingerprint in HONDA_BOSCH:
          ...
          if (not CS.out.gasPressed) and (actuators.longControlState == LongCtrlState.pid) and \
            ...
            self.brakefactor = np.clip(self.brakefactor - self.gas_error / 75 * (apply_brake * 4.8), 1.0, 3.0) # 25 after integral fix
